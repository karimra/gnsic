...
func (a *App) InitCertzInfoFlags(cmd *cobra.Command) {
	cmd.ResetFlags()
	//
	cmd.Flags().StringVarP(&a.Config.CertzInfoCertificate, "certificate", "i", "", "certificate file path")
	//
...
func (a *App) RunECertzInfo(cmd *cobra.Command, args []string) error {
	certPEM, err := os.ReadFile(a.Config.CertzInfoCertificate)
	if err != nil {
		return err
	}
...
func dsaKeyPrinter(name string, val *big.Int, buf *strings.Builder) {
...
func printVersion(version int, buf *strings.Builder) {
...
func printSubjectInformation(subj *pkix.Name, pkAlgo x509.PublicKeyAlgorithm, pk interface{}, buf *strings.Builder) error {
	buf.WriteString(fmt.Sprintf("%8sSubject: ", ""))
	buf.WriteString(subj.ToRDNSequence().String())
	buf.WriteString("\n")
...
func printSubjKeyId(ext pkix.Extension, buf *strings.Builder) error {
...
func printSubjAltNames(ext pkix.Extension, dnsNames []string, emailAddresses []string, ipAddresses []net.IP, buf *strings.Builder) error {
...
func printSignature(sigAlgo x509.SignatureAlgorithm, sig []byte, buf *strings.Builder) {
